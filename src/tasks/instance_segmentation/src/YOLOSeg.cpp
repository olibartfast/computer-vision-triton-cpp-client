                                         ...
                                         const std::vector<std::vector<int64_t>>& infer_shapes) 
{
    ...
    if(infer_shape[2] < infer_shape[1])
    {
        ...
        for (auto it = infer_result.begin(); it != infer_result.end(); it += (numClasses + 5 + maskDim))
        {
            ...
            if (clsConf > confThreshold)
            {
                ...
                {
                    float classConf = get_float(*(it + 5 + i));
                ...
                std::vector<float> box_coords(4);
                std::transform(it, it + 4, box_coords.begin(), get_float);
                ...
                std::vector<float> mask_coeffs(maskDim);
                std::transform(it + 5 + numClasses, it + 5 + numClasses + maskDim, mask_coeffs.begin(), get_float);
                picked_proposals.emplace_back(std::move(mask_coeffs));
            }
        }
    }
    ...
    std::vector<float> mask_data(mask_result.size());
    std::transform(mask_result.begin(), mask_result.end(), mask_data.begin(), get_float);
    cv::Mat protos(sc, sh * sw, CV_32F, mask_data.data());
