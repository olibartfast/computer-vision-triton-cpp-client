...
std::vector<uint8_t> DFine::preprocess_image(const cv::Mat& img, const std::string& format, int img_type1, int img_type3,
...
std::vector<std::vector<uint8_t>> DFine::preprocess(const std::vector<cv::Mat>& imgs)
{
    if (imgs.empty()) {
