...
DFine::DFine(const std::vector<std::vector<int64_t>>& input_sizes) : TaskInterface(input_sizes) {
    const auto image_layer = input_sizes[1];
    input_height_ = image_layer[3];
    input_width_ = image_layer[2];
    
}   

std::vector<uint8_t> DFine::preprocess(const cv::Mat& img, const std::string& format, int img_type1, int img_type3,
