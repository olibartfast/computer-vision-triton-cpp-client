...
ViTClassifier::ViTClassifier(const TritonModelInfo& model_info)
    ...
    Logger::getInstance().log(LogLevel::INFO, 
        "ViT Classifier initialized with input: " + model_info.input_names[0] + 
        ", output: " + model_info.output_names[0]);
    ...
    Logger::getInstance().log(LogLevel::INFO, 
        "Input shape: [" + std::to_string(input_shape[0]) + ", " + 
...
std::vector<uint8_t> ViTClassifier::preprocess_image(const cv::Mat& img, 
    ...
    Logger::getInstance().log(LogLevel::DEBUG, 
        "Preprocessed image: " + std::to_string(img_size.width) + "x" + 
        std::to_string(img_size.height) + ", format: " + format + 
        ", data size: " + std::to_string(input_data.size()) + " bytes");
...
std::vector<std::vector<uint8_t>> ViTClassifier::preprocess(const std::vector<cv::Mat>& imgs) {
    ...
    Logger::getInstance().log(LogLevel::INFO, 
        "Processing " + std::to_string(imgs.size()) + " images, target size: " + 
...
std::vector<Result> ViTClassifier::postprocess(const cv::Size& frame_size,
    ...
    Logger::getInstance().log(LogLevel::DEBUG, 
        "Processing " + std::to_string(logits.size()) + " class logits");
    ...
    for (size_t i = 0; i < max_predictions; ++i) {
        ...
        Logger::getInstance().log(LogLevel::DEBUG, 
            "Top " + std::to_string(i + 1) + ": class " + 
            std::to_string(class_idx) + " with confidence " + 
            std::to_string(confidence));
    }
    
    Logger::getInstance().log(LogLevel::INFO, 
        "Generated " + std::to_string(results.size()) + " classification results");
